...
Each successful scan also publishes a bounded pregame pick board from immutable accepted prediction rows. Before one hour to kickoff, the board retains the newest approved capture per player and labels its recorded price for repricing. Exactly 60 minutes before kickoff, that selection freezes; captures after the cutoff and all in-game lines are excluded. The separate **Past results** view starts only with these prospective records and shows a win, loss or push only after the existing source-bound final-stat verifier reproduces it. Missing or unverified results, including voids without reproducible final-stat evidence, stay pending. The public API recomputes record identities, lock times, eligibility, totals and win rate before displaying them.
...
- Every successfully evaluated selection is audited, including rejected estimates. Scanner model versions from `empirical-v2` onward carry verified provider-event and normalized quote-row commitments plus an explicit generation time; advancing the current version does not remove those historical requirements. The ledger recomputes the row commitment from the retained normalized quote before accepting it into metrics. Incomplete or mismatched evidence cannot enter metrics. Missing samples, expired quotes, started events, and risk gates cannot recommend stakes.
- ROI, hit rate, Brier score, log loss, calibration bins and CLV use explicit denominators and real settlements. No result is inferred from missing data. Pending, push and void outcomes are distinct. Malformed retained payloads are excluded without suppressing the remaining cohort, and malformed outcomes stay unverified and pending. Recommendation-only replay requires literal acceptance plus a positive recorded stake no larger than the 5% reservation ceiling.
- Hit rate and each populated calibration bin include a nominal 95% Wilson interval, and reports include fixed 10-bin calibration error. Reports also provide deterministic 95% game-cluster robust t intervals for ROI, hit rate, Brier score, log loss and CLV when at least two distinct game IDs are available. These allow arbitrary dependence among selections within a game; they do not adjust for the same player appearing across games. The point estimates remain available alongside the intervals.
- Hosted performance snapshots are filtered to the exact model version used by the scanner. The dashboard displays all eligible forecasts at unit stakes separately from accepted recommendations at their recorded stake fractions; unavailable recommendation results remain unavailable. The audit CLI can still inspect another explicit version or the combined history.
- The public metrics API validates cohort identity, count relationships, numeric bounds, calibration structure, model identifiers and complete cluster fields, then returns a fixed allowlist. Internal snapshot fields and stored explanatory text never cross that boundary. Older snapshots may omit newer derived intervals; the API reports those fields as unavailable until a worker publishes a current report.
- Daily grading examines only the bounded dates supplied by the schedule/catch-up planner. It requires an exact final ESPN schedule team/date match, exactly one player-ID/stat match, and the stat row's team to be one of that game's participants; the observed ESPN-to-nflverse aliases `LAR`/`LA` and `WSH`/`WAS` are explicit. Daily modes always use the previous seven Eastern dates and add at most seven unresolved older dates per run from a bounded 30-day ledger window, oldest first; monitor modes retain the current three-day schedule window. The dashboard receives only yesterday/today/tomorrow even when grading uses older evidence. New NBA.com and nflverse rows carry a canonical batch commitment and a reproducible typed settlement-row hash; ESPN fallback rows retain the raw-response hash and add the same row hash. The current NFL hash commits passing, rushing and receiving yards plus receptions. Legacy NFL hashes remain valid for the three yard markets, but cannot settle receptions because they did not commit that value. Grading recomputes the row hash and retains the canonical schedule/stat evidence with provider, both hashes, observation time, stat-row identity, actual value, and final schedule identity. Performance reporting re-hashes that retained proof and recomputes the outcome from the exact stat, line, and side. Missing, manual, legacy, tampered, pregame, future, DNP, ambiguous, and nonfinal evidence stays pending. Canonical batch commitments bind normalized ingested records but cannot authenticate provider origin or replay unavailable raw provider bytes. This is observed-stat backtest grading; it does not confirm a sportsbook account settlement or cross-venue rule equivalence.
...
uv run python -m sportsbet.ledger --model-version empirical-jeffreys-v4
```
...
`PUBLIC_DATA_PIPELINE_ENABLED=true`. The independent **Public market data**
workflow attempts `public_daily` at 13:17 and 13:47 UTC and `public_monitor` at
:07/:22/:37/:52 each hour. It has its own concurrency queue, receives only the
