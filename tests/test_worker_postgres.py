...
def test_nba_provenance_recovery_uses_postgres_and_preserves_model_values(tmp_path,monkeypatch,row_count):
    ...
    try:
        with engine.begin() as conn:
            ...
            conn.exec_driver_sql(f'CREATE TABLE {schema}.nba_player_gamelogs '
                '(LIKE public.nba_player_gamelogs INCLUDING DEFAULTS INCLUDING INDEXES)')
            # Reproduce legacy rows predating the NOT VALID migration. Never weaken
            # the real table or insert invalid rows through an active constraint.
            conn.execute(table.insert(),[dict(row,source_provider='nba',
                source_observed_at=observed-timedelta(days=1)) for row in rows])
            definition=conn.exec_driver_sql("SELECT pg_get_constraintdef(oid) FROM pg_constraint "
                "WHERE conrelid='public.nba_player_gamelogs'::regclass "
                "AND conname='ck_nba_gamelog_source_evidence'").scalar_one()
        ...
        with engine.begin() as conn:
            ...
            conn.exec_driver_sql(f'ALTER TABLE {schema}.nba_player_gamelogs '
                'VALIDATE CONSTRAINT ck_nba_gamelog_source_evidence')
