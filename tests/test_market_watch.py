...
def test_prop_handoff_keeps_only_normalized_worker_context_and_exact_team_aliases():
    ...
    inventory={'quotes':[{'ticker':'PROP','player_target_id':'player','milestone_id':'game',
        ...
        'settlement_terms':{'schema_version':1,
            'rules_sha256':market_watch.digest({'primary':'Exact primary rule.','secondary':'Exact secondary rule.'}),
            'participation':'active_no_snap_fair_market_price','statistic':'after_one_snap_recorded_stat',
            'overtime':'unspecified','stat_corrections':'unspecified','stat_source':'unspecified',
            'classified':True}}],
    ...
    assert 'settlement_terms' not in handoff['evidence']['quotes'][0]
...
@pytest.mark.asyncio
async def test_cfb_publish_updates_schedule_alongside_market_summary(monkeypatch,tmp_path):
    from unittest.mock import AsyncMock
    from sportsbet import market_watch
    stored={};monkeypatch.chdir(tmp_path)
    schedule=AsyncMock(return_value='partial')
    monkeypatch.setattr(market_watch,'publish_slate',schedule)
    monkeypatch.setattr(market_watch,'publish_snapshot',lambda key,value:stored.update({key:value}))
...
async def test_cfb_slate_cache_avoids_repeated_espn_requests(monkeypatch):
    ...
    publisher=AsyncMock(return_value='cached')
    monkeypatch.setattr(market_watch,'publish_slate',publisher)
    assert await market_watch.publish_cfb_slate(now)=='cached'
    publisher.assert_awaited_once_with('cfb',now)
