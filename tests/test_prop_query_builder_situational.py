...
def test_teammate_out_name_without_identity_is_ignored() -> None:
    """A current name and report date cannot prove historical participation."""
    from sportsbet.prop.query_builder import PropQueryBuilder

    params = _nfl_params(teammate_out=["1628384"])
    sql, args = PropQueryBuilder.build(params)

    baseline_sql,baseline_args=PropQueryBuilder.build(_nfl_params())
    assert (sql,args)==(baseline_sql,baseline_args)
...
def test_teammate_out_contexts_uses_exact_snap_identity() -> None:
    ...
    params = _nfl_params(
        teammate_out_contexts=[{"name":"Patrick Mahomes","team":"KC","position":"QB",
                                "pfr_player_id":"MahoPa00","unit":"offense"}]
    )
    sql, args = PropQueryBuilder.build(params)
    ...
    assert "nfl_snap_counts" in sql, "teammate_out_contexts must reference snap evidence"
    assert "offense_snaps > 0" in sql
    assert "AND EXISTS" in sql.upper(), "missing team snap data must not count as absence"
    assert "source_record_sha256" in sql, "snap participation must be source committed"
    ...
    assert "MahoPa00" not in sql, "Raw player identity must not appear in SQL string"
    assert "KC" in args, "Team must be in args tuple"
    assert "MahoPa00" in args, "Player identity must be in args tuple"


def test_teammate_out_contexts_skips_unverified_identity() -> None:
...
def test_nba_teammate_name_without_identity_is_ignored() -> None:
    """Name-only NBA availability never defines a historical cohort."""
    from sportsbet.prop.nba_query_builder import NBAQueryBuilder
    ...
    assert "Anthony Davis" not in args, "Unverified player name must not affect query args"
