...
export default function PickCard({signal,alternatives=[],now,onExplain}:{signal:EVSignal;alternatives?:EVSignal[];now:number;onExplain:(signal:EVSignal)=>void}) {
  ...
  async function copy() {
    const current=pickTerms(signal);
  ...
  return <article className={styles.betCard} aria-label={`${pick.player} qualified pick`}>
    <header><PlayerPortrait signal={pick} /><div><h3>{pick.player}</h3><p>{pick.player_profile?.team ?? pick.availability?.team}{pick.player_profile?.jersey ? ` / #${pick.player_profile.jersey}` : ''}{pick.player_profile?.position ? ` / ${pick.player_profile.position}` : ''}</p></div><span className={styles.qualified}>Qualified</span></header>
    <h4>{terms.pick}</h4><div className={styles.betPrice}><strong>{pick.sportsbook}</strong><b>{terms.price}</b></div>
