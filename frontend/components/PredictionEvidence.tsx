  ...
  return <section ref={panel} tabIndex={-1} className={styles.predictionEvidence} aria-label={`Why ${signal.player} was forecast`}>
    ...
    <div className={styles.evidenceColumns}>
      ...
      <div><h3>Injuries & teammates</h3>
        ...
        <p>{availability?.roster_confirmed ? `Confirmed on the current ${availability.team} roster. ` : 'Current roster identity is not confirmed. '}An unlisted injury does not confirm game-day participation. Final starters, minutes and snap counts are not verified here.</p>
        {availability?.status==='observed' && <>
          <p><a href={availability.source_url} target="_blank" rel="noreferrer">{availability.source_url===availability.roster_source_url ? 'Roster injury report' : 'ESPN injury report'}</a> · <a href={availability.roster_source_url} target="_blank" rel="noreferrer">Current roster</a><br />Captured {time(availability.captured_at)}</p>
