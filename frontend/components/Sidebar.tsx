...
export default function Sidebar({activeView,onViewChange}:SidebarProps) {
  ...
  return <aside className={styles.sidebar}><Link className={styles.terminalBrand} href="/" aria-label="Return to Quant home"><span>Q</span><div><strong>Quant</strong><small>Decision desk</small></div></Link>
    <nav className={styles.sideNav} aria-label="Research views">{NAV.map(button)}<details className={styles.advancedNav}><summary>More tools</summary><div>{TOOLS.map(button)}</div></details></nav>
