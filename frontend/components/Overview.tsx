...
export default function Overview({sport,onOpenMarkets,onOpenPlayers}:{sport:Sport;onOpenMarkets:()=>void;onOpenPlayers:(player?:string)=>void}) {
  ...
  const load=useCallback(async()=>{
    ...
    try {
      const results=await Promise.allSettled([fetchForecasts(sport,signal,'qualified',fetch,1000,undefined,true),fetchForecasts(sport,signal,'candidates',fetch,1000),fetchForecasts(sport,signal,'library',fetch,36)]);
  ...
  const pickGroups=bestPickGroups(qualified,now),picks=pickGroups.map(group=>group.pick),records=publicSignals(forecasts,now).signals;
  ...
  const explained=[...picks,...watchRecords,...records].find(signal=>signal.id===selected);
  ...
  return <section className={styles.overview} aria-label={`${sport.toUpperCase()} decision desk`}>
    <header className={styles.hero}><div><span className={styles.kicker}>{sport.toUpperCase()} / PREGAME PLAYER PROP DESK</span><h1>{horizon}, without the noise.</h1><p>Fresh qualified picks first, then the strongest source-backed watchlist when prices need to be checked again. Every market locks at kickoff.</p></div><div className={styles.heroActions}><button type="button" className={styles.refresh} disabled={refreshing} onClick={()=>void load()}><RefreshCw size={16} className={refreshing ? styles.spinning : undefined} />Refresh</button><span>{checked ? `Checked ${new Date(checked).toLocaleTimeString([],{hour:'numeric',minute:'2-digit'})}` : 'Checking published data'}</span></div></header>
