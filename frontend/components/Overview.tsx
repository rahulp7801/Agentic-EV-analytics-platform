...
import {bestPickGroups,compareQuality,conservativeMargin,groupAlternateLines,recentCandidateGroups,researchPlayerPreview,type PickGroup} from '@/lib/bestPicks';
import {fetchForecasts} from '@/lib/fetchForecasts';
import {publicSignals,forecastWindow} from '@/lib/signalMetrics';
import {PROP_LABELS,latestRecordedQuote,sportsbookLabel} from '@/lib/pickTerms';
...
export default function Overview({sport,onOpenMarkets,onOpenSlip,onOpenPlayers}:{sport:Sport;onOpenMarkets:()=>void;onOpenSlip:()=>void;onOpenPlayers:(player?:string)=>void}) {
  ...
  const [selected,setSelected]=useState<string|null>(null),[showStatus,setShowStatus]=useState(false);
  ...
  const players=researchPlayerPreview([...recentGroups.map(group=>group.pick),...previewMarkets.map(group=>group.pick)]);
  ...
  return <section className={styles.overview} aria-label={`${sport.toUpperCase()} decision desk`}>
    ...
    <details className={styles.deskDetails} onToggle={event=>setShowStatus(event.currentTarget.open)}><summary>Data status and market sources</summary>{showStatus && <><p>Source availability and price comparisons are separate from qualified player picks.</p><button type="button" className={styles.refresh} onClick={onOpenMarkets}>Inspect market sources <ArrowUpRight size={15} /></button></>}</details>
  </section>;
}
