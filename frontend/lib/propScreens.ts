...
const FEE_SCOPE = 'Hypothetical one-contract Kalshi fee scenario. Sportsbook limits, funding charges and actual fills are not included.';
...
function text(value: unknown, maximum=200): string {
  if (typeof value !== 'string' || !value.trim() || value.length>maximum
      || /[\u0000-\u001f]/.test(value)) throw new Error('Invalid prop screen');
  return value;
}
...
function leg(value: unknown) {
  ...
  if (item.venue === 'kalshi') {
    decimal(item.displayed_size,0,Number.POSITIVE_INFINITY,true);
    return {venue:'kalshi', ticker:text(item.ticker,128), side:text(item.side,10),
      cost:decimal(item.cost,0,1,true),observed_at:timestamp(item.observed_at)};
  }
  if (item.venue === 'sportsbook') return {
    venue:'sportsbook', sportsbook:text(item.sportsbook,64), side:text(item.side,10),
...
function feeScenarios(value: unknown, kalshiCost: string, sportsbookCost: string) {
  ...
  if (scenario.schedule_ref !== 'https://kalshi.com/regulatory/fee-schedule')
    throw new Error('Invalid prop screen');
  return {direct:result.direct,non_direct:result.non_direct,scope:FEE_SCOPE};
}
...
export function publicPropScreen(value: unknown, now=Date.now()) {
  ...
  const comparisons=data.comparisons.map(value => {
      ...
      if ((item.kind !== 'kalshi_sportsbook_prop' && item.kind !== 'sportsbook_sportsbook_prop')
          ...
          || item.reasons.length === 0 || item.reasons.length>4) {
        throw new Error('Invalid prop screen');
      }
      ...
      return {kind:item.kind,event_id:text(item.event_id,128), player:text(item.player,100), prop_type:item.prop_type as string,
        line, gross_cost_to_one_dollar:cost, gross_gap_to_one_dollar:gap, legs,
        ...(modeled_fee_costs ? {modeled_fee_costs} : {}),settlement_review,
        limitations:item.reasons.map(value=>text(value,300)),
