...
export async function liveSchedule(sport:Sport,now=Date.now()) {
  const today=new Intl.DateTimeFormat('en-CA',{timeZone:'America/New_York',year:'numeric',month:'2-digit',day:'2-digit'}).format(now);
  const captured_at=new Date(now).toISOString();
  const days=[[-1,'Yesterday'],[0,'Today'],[1,'Tomorrow']] as const;
  const results=await Promise.allSettled(days.map(async ([offset,label])=>{
    ...
    const validated=scheduleSnapshot({sport,status:'complete',partial:false,as_of_date:today,captured_at,games},sport,now);
    if(validated.status!==200) throw new Error('Invalid schedule');
    return games;
  }));
  const available=results.filter(result=>result.status==='fulfilled');
  if(!available.length) return scheduleSnapshot(null,sport,now);
  ...
  return scheduleSnapshot({sport,status:partial?'partial':'complete',partial,as_of_date:today,captured_at,
    games:available.flatMap(result=>result.value)},sport,now);
}
