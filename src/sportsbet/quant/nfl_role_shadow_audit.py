"""Read-only prospective scoring of frozen role-history shadows; no candidate bets."""
...
from sportsbet.quant.nfl_role_shadow import (ARTIFACT_SHA256,FROZEN_AT,MARKETS,POLICY,VERSION,
...
from sportsbet.quant.nfl_role_shadow import REASONS as UNAVAILABLE_REASONS


def report_role_history_shadow(rows: list[dict], sport: str) -> dict:
    ...
    attempts=[]
    for row in rows:
        shadow=row.get('role_history_shadow')
    ...
    for prop in sorted(set(MARKETS.get(sport,{}))|{row['prop_type'] for row in selected.values()}):
        ...
        for row in cohort:
            ...
            if probability is None:
                ...
                reason=row['role_history_shadow'].get('reason','invalid_shadow_record')
...
def publish_role_history_shadow(rows: list[dict], sport: str, publish) -> None:
    """Research diagnostics cannot change the primary pipeline's success state."""
    import structlog
    try:
        result=report_role_history_shadow(rows,sport)
    except Exception as exc:
        result=dict(status='unavailable',error_type=type(exc).__name__,promote=False)
    try:
        publish(f'role-shadow-validation:{sport}',result)
    except Exception as exc:
...
def main():
    ...
    try:
        ...
        result={sport:report_role_history_shadow(rows,sport) for sport in sports}
