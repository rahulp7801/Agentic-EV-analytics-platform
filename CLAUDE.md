...
- Known measurement debt: `ev_percentage` currently means probability edge, not expected ROI. Backtest CLV conflates model and entry probabilities; replay compares a quote to itself. Frontend PropsAnalysis invents +/-5 percentage-point confidence intervals. Do not treat these as validated performance metrics.
- Known modeling debt: defense and pace are neutral constants pending real data, and scanner "inactives" are inferred from missing prop listings. Under signals bypass graph risk gates. These are not validated statistical features or consistent portfolio controls.
- Pricing correction: raw American price overrides stored/devigged probability for break-even, Kelly, and expected return. Legacy snapshots with no price infer payout from their break-even probability; synthetic provider prices are still not executable sportsbook quotes. `EVSignal.expected_return` and scanner JSON `expected_return` are binary/no-push returns; `ev_percentage`/`ev_pct` retain their legacy probability-edge meaning until the UI/schema migration.
- Scanner passes its selected Over quote into the graph so evaluated and exported sportsbook/price agree. Over analysis excludes explicit Under quotes; legacy unlabelled snapshots remain accepted. Under sizing now shares the 25% hard cap and configured Kelly fraction. Existing cached DB signals retain historical Kelly values: use `--force` for corrected scans.
...
- Removed the offensive-scoring "defensive rating" proxy and its season-total query. Context now uses pre-game logs for rest/home and neutral defense/pace values. Real timestamped team defensive efficiency is still needed; home/rest adjustments remain heuristics.
