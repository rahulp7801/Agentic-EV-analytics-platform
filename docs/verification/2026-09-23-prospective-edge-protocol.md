...
The daily artifact includes `evidence-status.json` with one of `insufficient_data`, `uncertainty_unavailable`, `edge_not_demonstrated`, or `prospective_hypothetical_edge_evidence`. The last state requires both predeclared uncertainty tests and all three sample floors. It still describes hypothetical recorded prices, because no execution evidence is present.

## Frozen shadow challenger

A stronger Beta(40,40) prior over the same player-history wins (`prior-80`) is scored beside v4 in the daily artifact. It was selected after inspecting the old 16-game slate; that slate is exploratory, with paired Brier 0.24861 versus market 0.24514. The candidate was frozen while the post-merge ledger still had zero forecasts. It changes neither recommendation probability nor stake. Future exact-quote games determine whether its paired Brier interval improves on market; a separately frozen selection policy and later priced outcomes would still be needed for a betting-edge claim. See the [player-history ablation](2026-09-23-player-history-ablation.md).
